;if(CloudflareApps.matchPage(CloudflareApps.installs['thZYYi13s7LW'].URLPatterns)){(function(){'use strict'
...
var options=window.CloudflareApps.installs['thZYYi13s7LW'].options
...
if("thZYYi13s7LW"==='preview')updateVisibility()}
function updateVisibility(){clearTimeout(visibilityTimeout)
var visible=true
var stored=false
try{stored=JSON.parse(store['cf-flashcard'])}catch(e){}
if("thZYYi13s7LW"==='preview'||!stored){visible=true}else{var timeframe=times[options.renew]
...
window.CloudflareApps.installs['thZYYi13s7LW'].scope={updateOptions:function updateOptions(nextOptions){clearTimeout(visibilityTimeout)
options=nextOptions
updateContent()
app.setAttribute('data-visibility','visible')},updateDelay:function updateDelay(nextOptions){options=nextOptions
closeApp()},updatePause:function updatePause(nextOptions){app.setAttribute('data-visibility',nextOptions.paused?'hidden':'visible')}}}())}